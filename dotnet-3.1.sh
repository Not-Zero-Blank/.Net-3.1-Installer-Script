...
if command=$(sudo apt-get update; \
  ...
  sudo apt-get update && \
  sudo apt-get install -y dotnet-sdk-3.1); then
...
if command=$(sudo apt-get update; \
  sudo apt-get install -y apt-transport-https && \
  sudo apt-get update && \
  sudo apt-get install -y aspnetcore-runtime-3.1); then
...
if command=$(sudo apt-get install -y dotnet-runtime-3.1); then
...
echo "Successfully Installed NET 3.1!"
